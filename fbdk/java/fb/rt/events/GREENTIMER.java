...
/** FUNCTION_BLOCK GREENTIMER (* Define minimum green times *)
  * @author JHC
  * @version 20190327/JHC - Generated.
  */
...
/** The index (5) of state MinReached. */
public static final int INDEX_MinReached = 5;
...
void state_CD(){
   ...
   if(PEDWANTSTOCROSS.value==true)
     state_CheckMin();
   if(PEDWANTSTOCROSS.value==false)
     state_START();
}
...
/** The actions to take upon entering state MinReached. */
void state_MinReached(){
   eccState = INDEX_MinReached;
   alg_AKSJDKASJDKAS();
   if(YELLOWSET.value==false)
     state_YellowSet();
}
